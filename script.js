...
(() => {
  ...
  // Surfaces the real reason playback failed (missing/unsupported file)
  // instead of failing silently — check the browser console.
  bgMusic.addEventListener('error', () => {
    console.warn(
      `Music could not load from "music/song.mp3". ` +
      `Make sure the file exists at that exact path/name (case-sensitive on most web hosts).`
    );
  });
  ...
  function attemptPlay() {
    ...
    if (playPromise && playPromise.catch) {
      playPromise.catch((err) => {
        console.warn(`Music blocked on first attempt: ${err.name} — ${err.message}`);
        // Some browsers (notably iOS Safari) can reject the very first
        // play() call even inside a real tap. Retry once on the next
        // touch/click anywhere on the page rather than staying silent.
        const retry = () => {
          bgMusic.play().catch((e2) =>
            console.warn(`Retry also failed: ${e2.name} — ${e2.message}`)
          );
        };
